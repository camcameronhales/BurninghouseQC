...
from burninghouse_qc.transfer import FileSnapshot
...
def test_passing_file_still_gets_a_report(tmp_path):
    ...
    cfg = make_config(tmp_path, mode="report_only")
    source = tmp_path / "spot.mov"
    source.write_text("pretend video")
...
def test_route_renames_report_to_match_a_deduplicated_video(tmp_path):
    ...
    outcome = route(build_result(source, []), cfg)
    # The render itself is never filed, but its name is still reserved in the
    # verdict folder so a second render of the same name cannot overwrite the
    # first one's report.
...
def test_a_vanished_source_is_reported_not_crashed(tmp_path):
    ...
    assert outcome.report.exists(), "the report is written even so"
